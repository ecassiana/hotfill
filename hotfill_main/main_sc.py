...
def do_test_build(OPHS, tag, alg, alt, outfile):
  ...
  fph, BPHS = scanline.solve(OPHS, mp_jump, alt)
  end_time = time.time()
...
alg = arguments[2]
alt = False if arguments[2] == 'SCANLINE' else True
outfile  = arguments[3]

angle = 0 if "_0.txt" in partname else 1.5708
islice = int(partname.split("_")[1])
shift = (0, 0)

tag = "%s_slc%04d_%s" % (partname.replace(".txt", ""), islice, alg)
...
infolder = "tests/in/"
outfolder = "tests/out/" + partname.replace(".txt", "") + "_" + str(alg) + "/"
if not os.path.exists(outfolder):
  os.mkdir(outfolder)

fname_in = infolder + partname
fname_out = partname.replace(".txt", "") + "_" + str(alg) + "/" + outfile
rd = open(fname_in, 'r')
smptol = 0.0
fixdir = True
...
do_test_build(OPHS, tag, alg, alt, outfile)
